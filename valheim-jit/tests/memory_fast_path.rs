...
#[test]
fn native_dram_accesses_preserve_riscv_unaligned_semantics() {
  let mut cpu = RV64Cpu::new(None);
  let pc = VirtAddr(RV64_MEMORY_BASE);
  let data = VirtAddr(RV64_MEMORY_BASE + 0x103);
  ...
  cpu
    .bus
    .write::<u64>(data, 0x0123_4567_89ab_cdef)
    .unwrap();
...
#[test]
fn native_byte_access_at_page_end_never_takes_cross_page_slow_path() {
  let mut cpu = RV64Cpu::new(None);
  let pc = VirtAddr(RV64_MEMORY_BASE);
  let data = VirtAddr(RV64_MEMORY_BASE + 0xfff);
  cpu.bus.write::<u32>(pc, 0x0000_c103).unwrap(); // lbu x2, 0(x1)
  cpu
    .bus
    .write::<u32>(pc + VirtAddr(4), 0x0000_006f)
    .unwrap(); // jal x0, 0
  cpu.bus.write::<u8>(data, 0xa5).unwrap();
  cpu.write_reg(Reg::X(Fin::new(1)), data.0);
  let mut jit = JitExecutor::new().unwrap().with_hot_threshold(1);

  run_three_times(&mut jit, &mut cpu, pc, 2);

  assert_eq!(cpu.read_reg(Reg::X(Fin::new(2))), Some(0xa5));
  assert_eq!(jit.stats().memory_slow_paths, 0);
  assert_eq!(jit.stats().native_executions, 2);
}
